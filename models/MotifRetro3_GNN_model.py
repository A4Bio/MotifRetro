...
from modules.MotifRetro3_GNN_module import MeganDecoder
from modules.MotifRetro3_GNN_module import MeganEncoder
import numpy as np
import random
import time
import math
...
class MotifRetro_model(nn.Module):
    def __init__(self, 
        ...
        # self.action_embedding = nn.Embedding(n_atom_actions+n_bond_actions+1, hidden_dim)
        # self.time_embedding = nn.Linear(768, hidden_dim)
    ...
    def _preprocess_sparse(self,x):  
        ...
        # prev_action[:,0] = self.global_id
        # prev_t = torch.arange(prev_action.shape[1], device=prev_action.device).unsqueeze(0).repeat(prev_action.shape[0],1)
        # attention_mask = prev_action!=-1
        # action_embed = self.actionformer(prev_action, prev_t, attention_mask)
        # # atom_feats[x['supernode_idx']] += action_embed
        # uni_id, count = x['graph_id'].unique(return_counts=True)
        # atom_feats += action_embed.repeat_interleave(count, dim=0)
