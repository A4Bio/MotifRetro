...
class MultiHeadGraphConvLayer(nn.Module):
    ...
    def SynthonAttn(self, atom_feats, sub_graph_id, not_padding, graph_id, hidden_feat=None):
        ...
        atom_feats = self.synthon_input(torch.cat([atom_feats, hidden_feat], dim=-1))
        
        # synthon attention
        synthon_feat = scatter_mean(
...
class MeganEncoder(nn.Module):
    ...
    def forward(self, x: dict) -> dict:
        ...
        edge_idx = x['edge_idx']
        sub_graph_id = x['sub_graph_id']
        graph_id = x['graph_id']
        not_padding = x['atom_mask']
        prev_atom_feats = atom_feats
        temporal_feat = torch.zeros_like(atom_feats)
...
class MeganDecoder(nn.Module):
    ...
    def forward(self, x: dict) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        scores = {"atom_idx": 
                   ...
                   "atom_action":atom_actions_feat.reshape(-1),
                   "atom_mask": x['atom_action_mask'].reshape(-1),
                   ...
                   "bond_action":bond_actions_feat.reshape(-1),
