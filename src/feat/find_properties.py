...
from src.feat.graph_features import try_get_atom_feature, try_get_bond_feature
...
def update_feat_values(mol: Mol, atom_props: dict, bond_props: dict):
    for atom in mol.GetAtoms():
        for prop_key in atom_props.keys():
            atom_props[prop_key].add(try_get_atom_feature(atom, prop_key))  # e.g. prop_key = 'atomic_num'
...
def find_properties_parallel(params) -> dict:
    thread_num, data_x, feat_loop, ATOM_PROPS, BOND_PROPS = params
    ...
    for sub_smi, prod_smi in feat_loop(zip(data_x['substrates'], data_x['product']),
        ...
        for smi in (sub_smi, prod_smi):
            ...
            try:
                mol = fix_explicit_hs(mol)  # fix bug from rdkit.
