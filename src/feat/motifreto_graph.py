...
from utils.megan_utils import to_torch_tensor, lists_to_tuple
...
class MotifRetroTrainingSamplesFeaturizer(ReactionFeaturizer):
    """
    ...
    def __init__(self, split: DatasetSplit, n_jobs: int = 1,
                 ...
                 use_motif_action: bool = False, use_motif_feature: bool = False, vocab_path: str = None):
        """
        ...
        """
        super(MotifRetroTrainingSamplesFeaturizer, self).__init__()
        ...
        # if use_motif:
        #     self.key += '_motif_{}'.format(vocab_path.split('/')[-1].split('.')[0])
        self.key = key
        ...
        if self._vocabulary is None:
            ...
            vocab = {
                ...
                'atom_actions': [action for action in action_tuples if action[0] == 'change_atom'
                                 or action[0] == 'add_ring' or action[0] == 'add_atom' or action[0] == 'add_motif' or action[0] == 'stop'],
    ...
    def featurize_dataset(self, dataset: Dataset):
        ...
        metadata = dataset.load_metadata()  # metadata includes reaction type labels and reaction ID
        reaction_type_given = False
        if 'reaction_type_id' in metadata:  # 预处理默认包括了 reaction type，只有在训练的时候才考虑是不是要加入这个特征
        ...
        prop_dict = {'atom': {}, 'bond': {}}   # result of find_properties_parallel
        # self.n_jobs = 1
        if self.n_jobs == 1:
        ...
        # prop_dict == chunk_results[0] is True if self.n_jobs == 1
        ...
        bond_feature_keys = [k for k in ORDERED_BOND_OH_KEYS if k in prop2oh['bond']]  # 'is_aromatic' feature is revmoed.
        action_vocab = {
            'prop2oh': prop2oh,
        ...
        for i, ch_inds in enumerate(chunk_inds):
            ...
            parallel_args.append((i, samples_len, ch_inds, new_x, max_n_nodes, tqdm,
                                  ...
                                  self.forward, self.action_order,  # self.action_order = 'bfs_randat'
        ...
        nodes_motif_mat = sparse.csr_matrix(([], ([], [])), shape=(samples_len, max_n_nodes))
        nodes_atom2motif_mat = sparse.csr_matrix(([], ([], [])), shape=(samples_len, max_n_nodes))
        adj_mat = sparse.csr_matrix(([], ([], [])), shape=(samples_len, max_n_nodes ** 2))  # [dataset_size, max_n_nodes, max_n_nodes]; symmetric matrix


        sample_col2name = {1: 'atom_map1', 2: 'atom_map2', 3: 'n_nodes', 4: 'is_hard', 5: 'reaction_type'}
        if self.use_motif_feature:
            sample_col2name[6] = 'atom_motif_id'

        n_sample_data = len(sample_col2name) + 1  # TODO: 还没考虑不使用 hier-motif 但 given reaction type 的情况
        ...
        for ch_inds, result_code, chunk_save_path in tqdm(zip(chunk_inds, chunk_results, chunk_save_paths),desc='merging reactions from chunks', total=self.n_jobs):
            ...
            if self.use_motif_feature: # TODO: 需要加上node2motif和motif2node的映射
                nodes_motif_mat_path = os.path.join(chunk_save_path, 'nodes_motif_mat.npz')
                nodes_motif_mat += sparse.load_npz(nodes_motif_mat_path)

                nodes_atom2motif_mat_path = os.path.join(chunk_save_path, 'nodes_atom2motif_mat.npz')
                nodes_atom2motif_mat += sparse.load_npz(nodes_atom2motif_mat_path)
        ...
        if self.use_motif_feature:
            sparse.save_npz(get_nodes_motif_path(feat_dir), nodes_motif_mat)
            sparse.save_npz(get_atom2motif_path(feat_dir), nodes_atom2motif_mat)
    ...
    def load(self, feat_dir: str) -> dict:
        ...
        if self.use_motif_feature:
            result = {
                **result,
                'nodes_motif': sparse.load_npz(get_nodes_motif_path(this_feat_dir)),
                'atom2motif': sparse.load_npz(get_atom2motif_path(this_feat_dir)),
            }
        return result

    # noinspection PyMethodOverriding
    def to_tensor_batch(self, data: dict, actions_vocab: dict, use_motif_feature=False) -> dict:
